...
function assertPubKey(pk: Uint8Array, idx: number): void {
  if (!(pk instanceof Uint8Array) || (pk.length !== 33 && pk.length !== 65)) {
    throw new Error(`pubKey[${idx}] must be 33 or 65 bytes (got ${pk?.length})`);
  }
}

export function encodeMultisigRedeemScript({ m, pubKeys }: MultisigParams): Uint8Array {
  if (!Array.isArray(pubKeys) || pubKeys.length === 0) {
    throw new Error('pubKeys must be a non-empty array');
  }
  if (pubKeys.length > MULTISIG_MAX_PUBKEYS) {
    throw new Error(
      `pubKeys length ${pubKeys.length} exceeds OP_CHECKMULTISIG cap (${MULTISIG_MAX_PUBKEYS})`
    );
  }
  if (!Number.isInteger(m) || m < 1 || m > pubKeys.length) {
    throw new Error(`m must be an integer in [1, ${pubKeys.length}] (got ${m})`);
  }
  for (let i = 0; i < pubKeys.length; i += 1) assertPubKey(pubKeys[i], i);

  const parts: Uint8Array[] = [pushInt(m)];
  for (const pk of pubKeys) parts.push(pushBytes(pk));
  parts.push(pushInt(pubKeys.length));
  parts.push(Uint8Array.of(OP_CHECKMULTISIG));
  return concatBytes(...parts);
}

export function encodeMultisigRedeemScriptHex(params: MultisigParams): string {
 ...
 */
export function encodeP2SHScriptPubKey(redeemScriptHash160: Uint8Array): Uint8Array {
