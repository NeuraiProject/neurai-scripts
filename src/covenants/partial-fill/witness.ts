 ...
 */
export function buildFillWitnessStack(amount: bigint, total: bigint): Uint8Array[] {
 ...
 */
export function buildCancelWitnessStack(
 ...
 */
export function buildCancelWitnessStackPQ(
  sigPQ: Uint8Array,
  pubKey: Uint8Array
): Uint8Array[] {
  ...
  if (sigPQ.length > MAX_PQ_SCRIPT_ELEMENT_SIZE) {
    throw new Error(
      `sigPQ of ${sigPQ.length} bytes exceeds MAX_PQ_SCRIPT_ELEMENT_SIZE (${MAX_PQ_SCRIPT_ELEMENT_SIZE})`
    );
  }
  ...
  if (pubKey.length > MAX_PQ_SCRIPT_ELEMENT_SIZE) {
    throw new Error(
      `pubKey of ${pubKey.length} bytes exceeds MAX_PQ_SCRIPT_ELEMENT_SIZE (${MAX_PQ_SCRIPT_ELEMENT_SIZE})`
    );
  }
