...
export function makeCursor(bytes) {
    return { bytes, pos: 0 };
}
/** Consume one byte and verify it equals `expected`. */
export function expectByte(c, expected, label) {
...
export function assertTrailing(c) {
    if (c.pos !== c.bytes.length) {
 ...
 * and payload ranges.
 */
export function readPush(c, label) {
    if (c.pos >= c.bytes.length) {
 ...
 */
export function decodeScriptNum(data, label) {
    if (data.length === 0)
        return 0n;
    if (data.length > 8) {
 ...
 */
export function readPushPositiveInt(c, label) {
    if (c.pos >= c.bytes.length) {
