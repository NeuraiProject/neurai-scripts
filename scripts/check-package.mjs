...
//   4. For every entry with a `require` condition, compare the values its
//      CommonJS module exports at runtime with the values its CommonJS
//      declarations (`.d.cts`) promise, using the TypeScript compiler API.
//
  ...
  for (const [subpath, conditions] of entries()) {
    ...
    if (conditions.import) {
      step(`runtime: import "${specifier}"`, () =>
        probe(`const m = await import(${JSON.stringify(specifier)}); console.log(Object.keys(m).length + " exports")`, "module"),
      );
    }
    if (conditions.require) {
      ...
      const requireTypes = typeof conditions.require === "object" ? conditions.require.types : undefined;
      step(`declarations: "${specifier}" require types match the CommonJS runtime`, () => {
        if (!requireTypes || !requireTypes.endsWith(".d.cts")) {
          throw new Error(`the require condition has no .d.cts declarations (${requireTypes ?? "none"})`);
        }
        const declared = declaredValueExports(join(app, "node_modules", pkg.name, requireTypes));
